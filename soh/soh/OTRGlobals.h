...
uint32_t OTRGetFramebufferWidth(int fb);
uint32_t OTRGetFramebufferHeight(int fb);
void OTRReadFramebufferPixels(int fb, uint32_t x, uint32_t y, uint32_t width, uint32_t height, uint32_t type, void* data);
